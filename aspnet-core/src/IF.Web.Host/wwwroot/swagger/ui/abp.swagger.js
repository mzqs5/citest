...
(function () {
    ...
    function loginUserInternal(tenantId, callback) {
        var usernameOrMobile = document.getElementById('userName').value;
        if (!usernameOrMobile) {
            alert('Username or Mobile is required, please try with a valid value !');
            return false;
        }
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    var result = responseJSON || responseJSON.result;
        ...
        xhr.send("{" + "usernameOrMobile:'" + usernameOrMobile + "'," + "password:'" + password + "'}");
    };
    ...
    abp.swagger.openAuthDialog = function (loginCallback) {
        ...
        createInput(modalUxContent, 'userName', 'Username or Mobile');
